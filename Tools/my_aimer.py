...
class PowerBtn(QtWidgets.QPushButton):
    rightClicked = QtCore.pyqtSignal()
    doubleClicked = QtCore.pyqtSignal()

    def __init__(self, parent=None):
        super(PowerBtn, self).__init__(parent)
...
class OpenSliderDialog(QtWidgets.QDialog):
    ...
    def create_widgets(self):
        ...
        self.make_loc_btn = PowerBtn(self)
    ...
    def create_connections(self):
        ...
        self.make_loc_btn.rightClicked.connect(self.get_target_locs)
        # Slider
    ...
    def radio_btn_context_menus_instanes(self):
        ...
        self.cb_widg_list = [self.axis_x_btn, self.axis_y_btn, self.axis_z_btn, self.axis_mx_btn, self.axis_my_btn,
                             self.axis_mz_btn]
    ...
    def get_target_locs(self):
        print("get target locs for sel")
        if self.targetLocList:
            print("self.targetLocList:", self.targetLocList)
            mc.select(self.targetLocList)
    ...
    def make_locators(self):
        ...
        for ctrl in self.chain_ctrls:
            temp_target_name = ctrl + "_temp_aim_target"
            # Delete if locators already exist
            if mc.objExists(temp_target_name):
                name_list = mc.ls(temp_target_name, long=1)
                mc.delete(name_list)

            temp_loc = mc.spaceLocator(n=temp_target_name)
    ...
    def make_rig(self):
        mc.evalDeferred(lambda: self.make_rig_setup())

    def make_rig_setup(self):
        ...
        # Baking
        mc.evalDeferred(lambda: self.myBake(self.bakees, space_obj=obj))

        mc.evalDeferred(lambda: self.post_bake_constraints(tempConstraints))

    def post_bake_constraints(self, tempConstraints):
